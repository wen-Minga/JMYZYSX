(function () {
  ...
  const syncHomePlasma = () => {
    ...
    if (!main || !projects || window.__cmcHomePlasmaActive) return;
    ...
    const render = (time) => {
      if (!visible) return;
      if (document.documentElement.classList.contains('cmc-detail-pinned') || document.documentElement.classList.contains('cmc-wall-open')) {
        frame = requestAnimationFrame(render);
        return;
      }
    ...
    const updateActive = () => {
      const active = !document.hidden
        ...
        && projects.getBoundingClientRect().top <= window.innerHeight * .62;
...
(function removeDepartmentContent() {
  ...
  const startFilmHeroTitleMotion = (title) => {
    ...
    const play = () => window.requestAnimationFrame(() => title.classList.add('is-drawn'));
    if (window.matchMedia('(prefers-reduced-motion: reduce)').matches || window.__cmcOpeningComplete) {
      play();
    } else {
      window.addEventListener('cmc:opening-complete', play, { once: true });
    }
  };
  ...
  const syncFilmHomeArchive = (about) => {
      ...
      const tick = (now) => {
        ...
        detailCards.forEach((card, index) => {
          ...
          const extraY = direction > 0 && fromDepth === 0
            ? Math.sin(Math.PI * raw) * detailDropDistance
            : 0;
    ...
    const releaseDetailWithOverflow = (overflow, complete) => {
      ...
      if (overflow) {
        (window.__cmcSmoothPageScrollTo || window.__cmcResetPageScroll)?.(Math.max(0, pinTop + overflow));
      }
    };
    ...
    const enforceDetailBoundary = () => {
      ...
      const scrollTop = window.scrollY;
      if (!detailPinned) {
        if (detailCycleComplete && scrollTop <= pinTop + 1) {
          detailStep = detailMaxStep;
          renderDetailStep(detailStep);
          detailCycleComplete = false;
          pinDetailStage();
        } else if (!detailCycleComplete && scrollTop >= pinTop - 1) {
          pinDetailStage();
        }
        return;
      }
      ...
      if (Math.abs(scrollTop - pinTop) > 16) window.__cmcResetPageScroll?.(pinTop);
    };
    const handleDetailWheel = (event) => {
      if (!detailStage || event.ctrlKey || event.metaKey || event.shiftKey) return;
      ...
      if (!detailPinned) {
        if (detailCycleComplete) {
          if (delta < 0 && scrollTop - Math.abs(delta) <= pinTop + 8) {
        ...
        if (delta > 0 && (scrollTop >= pinTop - 8 || scrollTop + Math.max(24, delta) >= pinTop - 4)) {
          consumeDetailInput(event);
          pinDetailStage();
        }
        return;
    ...
    const handleDetailTouchMove = (event) => {
      if (!detailStage || detailPointerLastY === null || !event.touches[0]) return;
      ...
      if (!detailPinned && delta > 0 && (scrollTop >= pinTop - 8 || scrollTop + Math.abs(delta) >= pinTop - 4)) {
        consumeDetailInput(event);
        pinDetailStage();
        detailTouchConsumed = true;
        return;
